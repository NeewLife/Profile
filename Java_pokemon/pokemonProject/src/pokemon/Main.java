...
public class Main {// class 시작
	...
	public static void main(String[] args) { // 메인 시작
		...
		// [스타팅 포켓몬 선택]
		selectStartPokemon(player, 꼬부기, 파이리, 파이어);
...
//				}
	...
	public static void enemaBattle(ArrayList<pokemon> myPokemonList, ArrayList<pokemon> enemylist) {
		...
		pokemon enemy = null;
		Loop1:
		for (int i = 0; i < myPokemonList.size() + 1; i++) {
			System.out.println("내고싶은 포켓몬을 선택해주세요!");
			timedelay(1000);
			System.out.println("--------[나의 포켓몬리스트]--------");
			for (int z = 0; z < myPokemonList.size(); z++) {
				System.out.printf("| %d-%s (현재 체력 : %d)", z + 1, myPokemonList.get(z).name,
						myPokemonList.get(z).presentHp);
			}
			System.out.println("|");
			System.out.println("-------------------------------");
			int choosepokemon = sc.nextInt();
